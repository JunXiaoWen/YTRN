...
class SequenceModule(nn.Layer):
    def __init__(self, in_channels=4, **kwargs):
        ...
        self.conv = nn.Sequential(
            ...
            BlockA(in_channel=32, out_channel=32),
            ...
            BlockA(in_channel=256, out_channel=256),
            BlockB(in_channel=256, out_channel=512),  # 25
            BlockA(in_channel=512, out_channel=512),
        )
    def forward(self, x):
        # x [b, c, w]
        y = self.conv(x)
...
class ResNet(nn.Layer):
    def __init__(self, in_channels=7, layers=50, **kwargs):
        ...
        self.out_pool = nn.AdaptiveAvgPool2D((1, 20))
    ...
    def forward(self, inputs, seq):
        ...
        # seq = self.sm(seq)
        # seq = seq.unsqueeze(2)
        # inputs = paddle.concat([inputs, seq], 1)
...
if __name__ == '__main__':
    ...
    x = paddle.randn((2, 7, 32, 256), dtype='float32')
    seq = paddle.randn((2, 4, 400), dtype='float32')
    sm = SequenceModule(x)
    res = ResNet(in_channels=7, layers=34)
    ba = BlockA(in_channel=4, out_channel=4)
    bb = BlockB(in_channel=4, out_channel=4, stride=1)
    # y = res(x, seq)
    # print(y.shape)
    paddle.summary(res, ((1, 7, 32, 128), (1, 4, 400)))
