...
def train(config,
    ...
    if type(eval_batch_step) == list and len(eval_batch_step) >= 2:
        ...
        if len(valid_dataloader1) == 0:
            logger.info(
                'No Images in eval dataset1, evaluation during training ' \
                'will be disabled'
            )
            start_eval_step = 1e111
        if len(valid_dataloader2) == 0:
            logger.info(
                'No Images in eval dataset2, evaluation during training ' \
                'will be disabled'
            )
            start_eval_step = 1e111
        logger.info(
